...
function createUser(nome, senha){//funcional
    if(verifica()==true ){
        ...
        localStorage.setItem('School_Game_User:'+user.name, JSON.stringify(user));//salva m objeto no local storage
        }else {
...
formBtn2.addEventListener('click', getUser(inputName.value))
